...
// Instance Methods
eventSchema.methods.addRSVP = function (rsvpData) {
    // Check if user already has an RSVP
    const existingRSVP = this.rsvps.find((rsvp) => (rsvpData.userId && rsvp.userId?.equals(rsvpData.userId)) ||
...
eventSchema.methods.updateRSVP = function (userId, status) {
    const rsvp = this.rsvps.find((r) => r.userId?.equals(userId));
    if (rsvp) {
        rsvp.status = status;
...
eventSchema.methods.getWaitlistCount = function () {
    return this.rsvps.filter((rsvp) => rsvp.status === 'waitlist').length;
};
...
eventSchema.methods.canAcceptMoreAttendees = function () {
    if (!this.registrationSettings.maxAttendees)
        return true;
    const currentAttendees = this.getAttendeeCount();
    return currentAttendees < this.registrationSettings.maxAttendees;
};
eventSchema.methods.sendAnnouncement = function (announcement) {
    this.announcements.push(announcement);
    return this.save();
};
eventSchema.methods.calculateAttendanceRate = function () {
    const attendingCount = this.rsvps.filter((rsvp) => rsvp.status === 'attending').length;
    const checkedInCount = this.rsvps.filter((rsvp) => rsvp.checkInStatus === 'checked-in').length;
    if (attendingCount === 0)
        return 0;
...
eventSchema.statics.getEventsByDate = function (startDate, endDate) {
    return this.find({
        $or: [
...
eventSchema.statics.getEventStatistics = async function () {
    const now = new Date();
    const [totalEvents, upcomingEvents, categoryStats, monthlyStats, attendanceStats] = await Promise.all([
...
eventSchema.statics.getOrganizerEvents = function (organizerId) {
    return this.find({
        $or: [
