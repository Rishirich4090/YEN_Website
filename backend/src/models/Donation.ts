...
// Instance Methods
DonationSchema.methods.calculateNetAmount = function(): number {
  return this.amount - (this.processingFee || 0);
};

DonationSchema.methods.generateTaxReceipt = async function(): Promise<string> {
...
DonationSchema.methods.sendThankYouEmail = async function(): Promise<boolean> {
  try {
...
DonationSchema.methods.sendCertificate = async function(): Promise<boolean> {
  try {
...
DonationSchema.methods.processRefund = async function(reason?: string): Promise<IDonation> {
  this.paymentStatus = 'refunded';
  this.notes = `${this.notes || ''}\nRefund processed: ${reason || 'No reason provided'}`;
  return this.save();
};

DonationSchema.methods.markAsVerified = function(verifiedBy: mongoose.Types.ObjectId): Promise<IDonation> {
...
// Static Methods
DonationSchema.statics.getTotalDonations = async function(startDate?: Date, endDate?: Date): Promise<number> {
  const query: any = { paymentStatus: 'completed' };
  
  if (startDate || endDate) {
    query.donationDate = {};
    if (startDate) query.donationDate.$gte = startDate;
    if (endDate) query.donationDate.$lte = endDate;
  }
...
DonationSchema.statics.getDonationsByProject = function(projectId: mongoose.Types.ObjectId): Promise<IDonation[]> {
  return this.find({ project: projectId, paymentStatus: 'completed' })
    .sort({ donationDate: -1 })
    .populate('project', 'title description');
};

DonationSchema.statics.getDonationsByDonor = function(donorEmail: string): Promise<IDonation[]> {
  return this.find({ donorEmail, paymentStatus: 'completed' })
    .sort({ donationDate: -1 });
};

DonationSchema.statics.getMonthlyRecurring = function(): Promise<IDonation[]> {
  return this.find({
...
DonationSchema.statics.getStatistics = async function(year?: number): Promise<{
  total: number;
  totalAmount: number;
  byType: Record<string, number>;
  byProject: Record<string, number>;
...
DonationSchema.statics.getUnsentCertificates = function(): Promise<IDonation[]> {
  return this.find({
...
DonationSchema.statics.getUnsentTaxReceipts = function(): Promise<IDonation[]> {
  return this.find({
    paymentStatus: 'completed',
    taxDeductible: true,
